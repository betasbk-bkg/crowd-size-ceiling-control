...
def _honest_block(iang, pang, tr, N, rng):
    ...
    if na + ns + no + nt != N:
        # v2.0.0 OVERFLOW GUARD (2026-07): the original allocation overflows at
        # (N=5, tr=0.30): na=3, ns=1, nt=2 -> 6 participants instead of 5
        # ('no' clamps at 0 and cannot absorb the negative remainder). On the
        # campaign grid this is the only affected cell; all other cells keep the
        # original allocation bit-for-bit. Guarded fallback: adversary count
        # nt=round(N*tr) is canonical; the honest remainder N-nt is split in the
        # original accurate:slow:other proportions (70/95, 20/95, exact
        # remainder), any negative remainder absorbed into 'accurate'.
        # 12 cells / 600 runs re-simulated with this guard active
        # (analysis/bugfix_N5_tr30_rerun.py, data/campaign_main_mc50_fixed.json).
