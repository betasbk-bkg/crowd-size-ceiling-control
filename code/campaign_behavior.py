"""
campaign_behavior.py  —  #9 behavioural sensitivity panel [campaign.py 동일 엔진/seed]
================================================================
...
seed: campaign.make_seed 확장 (behaviour id 추가) — 충돌 없음
...
def gen_votes_behavior(iang, pang, tr, N, rng, model, behaviour, frame,
                       prev_pub_deg, troll_hold):
    """honest 그룹에 behaviour variant 적용 + troll 은 model(T0/T1/T2)."""
    sc=(1-tr)/0.95
    na=round(N*0.70*sc); ns=round(N*0.20*sc); nt=round(N*tr); no=max(0,N-na-ns-nt)
    prog=frame/FRAMES  # 0→1 시간 진행
    # 기본 정확 그룹 노이즈 폭
    acc_noise=3.0
    if behaviour=='B2':   acc_noise=3.0+9.0*prog          # fatigue: 3°→12°
    elif behaviour=='B4': acc_noise=8.0-5.0*prog          # learning: 8°→3°
    angs=np.empty(na+ns+no); i=0
    # 정확 그룹
    base=iang+rng.uniform(-acc_noise,acc_noise,na)
    if behaviour=='B3' and prev_pub_deg is not None:       # imitation: 30% 가 공개합의로 끌림
        k=int(na*0.30)
    ...
    if ns>0:
        ...
        if behaviour=='B1':                                # var-delay: 지연 분산 확대
...
def sim_behavior(traj,N,tr,seed,model,behaviour,speed=5.0,k_hold=10):
    ...
    for f in range(FRAMES):
        if f%VOTE_INT==0:
            ...
            votes,hold=gen_votes_behavior(iang,pang,tr,N,rng,model,behaviour,f,prev_pub,hold); pang=iang
...
def run():
    ...
    print(f"#9 behavioural panel: {total} runs (4traj×2N×2tr×3model×5behav×MC{MC})")
    ...
    json.dump({'config':{'MC':MC,'Ns':NS,'trolls':TRS,'models':MODELS,'behaviors':BEHAV,
                         ...
                         'note':'#9 behavioural sensitivity — variants on honest group'},
